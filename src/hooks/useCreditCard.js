...
export const useCreditCard = () => {
    ...
    const handleChange = e => {
        ...
        else if (name === DATE && DATE_REGEX.test(value) && value.length <= DATE_LENGTH) {
            if (value.length >= 1) setDate(value.match(TWO_DIGITS).join('/'));
            else setDate(value);    
        } 
        else if (name === CVC && CVC_REGEX.test(value) && value.length <= CVC_LENGTH) setCvc(value);
    }
